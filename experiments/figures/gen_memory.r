...
outputFilename = paste0(script.basename, "/fig_memory.png")
...
ggplot(df, aes(x=Software, y=Memory)) +
    geom_bar(stat="identity", width = 0.8, fill="steelblue2") + #, color="black"
    ...
    geom_text(aes(label = round(Memory, digits = 0)), vjust=-1, size=6)  +
