...
#ifndef MDBG_METAG_OverlapRemover3
#define MDBG_METAG_OverlapRemover3

#include "../Commons.hpp"


class OverlapRemover3 {
...
public:
	...
	OverlapRemover3(const string& inputDir, const string& inputFilenameContig, const string& outputFilenameContig, size_t kminmerSize, const int nbCores){
		_inputDir = inputDir;
	...
	class CountContigsFunctor {
		...
		OverlapRemover3& _parent;

		CountContigsFunctor(OverlapRemover3& parent) : _parent(parent){
		}
		...
		void operator () (const KminmerList& kminmerList) {
			...
			/*
			if(kminmerList._readIndex == 111105){
				cout << "lul" << endl;
	...
	class IndexContigsFunctor {
		...
		IndexContigsFunctor(OverlapRemover3& parent) : _parent(parent){
		}
	...
	class MapContigsFunctor {
		...
		MapContigsFunctor(OverlapRemover3& parent) : _parent(parent){
		}
		...
		void operator () (const KminmerList& kminmerList) {
			...
			if(kminmerList._readIndex % 10000 == 0) cout << "Map contigs: " << kminmerList._readIndex << endl;
			...
			while(true){
				...
				if(kminmerList._readIndex == 92635){
					cout << minimizers.size() << " " << contigOverlap.first << " " << contigOverlap.second << endl;
				}
				...
				minimizers.erase(minimizers.begin(), minimizers.begin() + overlapLeft);
				minimizers.resize(minimizers.size()-overlapRight);
				
				if(minimizers.size() <= _parent._kminmerSize+1) return;
				/*
				vector<MinimizerType> newMinimizers(minimizers.begin() + overlapLeft, minimizers.begin() + indexEnd);

				if(newMinimizers.size() <= _parent._kminmerSize) return;
		...
		pair<u_int32_t, u_int32_t> computeOverlaps(const u_int32_t referenceContigIndex, const vector<MinimizerType>& minimizers){
			...
			const u_int32_t referenceContigLength = minimizers.size(); //_parent._contigSizes[referenceContigIndex];
		...
		u_int32_t getMaxOverlapLeft(const vector<Anchor>& anchors){
			...
			u_int32_t nextIndex = 0;

			for(size_t i=0; i<anchors.size(); i++){

				if(anchors[i]._referencePositionIndex == nextIndex){
					overlapSize += 1;
					nextIndex += 1;
				}
			}
		...
		u_int32_t getMaxOverlapRight(const vector<Anchor>& anchors, const u_int32_t& referenceSize){
			...
			u_int32_t overlapSize = 0;
			u_int32_t nextIndex = referenceSize - 1 - _parent._kminmerSize + 1;
