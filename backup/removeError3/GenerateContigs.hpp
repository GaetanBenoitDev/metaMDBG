...
public:
	...
	void loadGraph(){
		...
		//_unitigDatas.resize(_graph->_graphSuccessors->_nodeDatas.size());
		//indexReads();
	...
	void indexReads(){
		...
		auto fp = std::bind(&GenerateContigs::indexReads_read, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4, std::placeholders::_5);
		parser.parse(fp);
	...
	void indexReads_read(const vector<u_int64_t>& minimizers, const vector<KmerVec>& kminmers, const vector<ReadKminmer>& kminmersInfos, bool isCircular, u_int64_t readIndex){//}, const vector<KmerVec>& kminmers_k3, const vector<ReadKminmer>& kminmersInfos_k3){
		...
		for(const KmerVec& vec : kminmers){
			...
			//cout << _unitigDatas.size() << " " << nodeName << endl;
