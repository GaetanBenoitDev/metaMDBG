...
struct LoopRepeat{
    vector<u_int32_t> _loopNodes;
    vector<u_int32_t> _repeatNodes;
};
...
struct SaveState2{
    ...
    phmap::parallel_flat_hash_map<u_int32_t, LoopRepeat> _repeatNodes;
    //vector<Bubble> _bubbles;
    //unordered_map<u_int32_t, u_int32_t> _longUnitigNodeAbundance;
};
...
public:
    ...
    vector<bool> _isNodeNameRemovedAndSaved;
    //unordered_set<u_int32_t> savedRemovedNodeNames;
    ...
    */
    ...
    void clear(float abundanceCutoff_min){
        ...
        _isNodeNameRemovedAndSaved = vector<bool>(_graphSuccessors->_nbNodes/2, false);
        //_isEdgeRemoved.clear();
        _bubbles.clear();
    ...
    */
    ...
    //struct LoopRepeat{
    //    u_int32_t _loopUnitigIndex;
    //    u_int32_t _repeatUnitigIndex;
    //    u_int32_t _sideUnitigIndexSucc;
    //    u_int32_t _sideUnitigIndexPred;
    //};

    u_int64_t removeLoopRepeat_withMiddle(float maxRepeatLength, SaveState2& saveState){

        unordered_set<u_int32_t> processedUnitigIndex;
        //vector<LoopRepeat> loopRepeats;
        unordered_set<u_int32_t> removedNodes;
        unordered_set<u_int32_t> removedUnitigs;
        ...
        for(const UnitigTip& unitigTip : unitigTips){
            ...
            u_int32_t loopUnitigIndex = unitigTip._unitigIndex;
            u_int32_t repeatUnitigIndex = -1;
            u_int32_t sideUnitigIndexSucc = -1;
            u_int32_t sideUnitigIndexPred = -1;



	        bool isRepeat = solveLoopRepeat_withMiddle(maxRepeatLength, loopUnitigIndex, repeatUnitigIndex, sideUnitigIndexSucc, sideUnitigIndexPred);
            //cout << isRepeat << endl;
            if(isRepeat){

                cout << "Found repeat!" << endl;

                if(processedUnitigIndex.find(loopUnitigIndex) != processedUnitigIndex.end()) continue;
                if(processedUnitigIndex.find(repeatUnitigIndex) != processedUnitigIndex.end()) continue;
                if(processedUnitigIndex.find(sideUnitigIndexSucc) != processedUnitigIndex.end()) continue;
                if(processedUnitigIndex.find(sideUnitigIndexPred) != processedUnitigIndex.end()) continue;

                processedUnitigIndex.insert(loopUnitigIndex);
                processedUnitigIndex.insert(repeatUnitigIndex);
                processedUnitigIndex.insert(sideUnitigIndexSucc);
                processedUnitigIndex.insert(sideUnitigIndexPred);
                //cout << BiGraph::nodeIndex_to_nodeName(_unitigs[loopUnitigIndex]._startNode) << endl;
                //cout << BiGraph::nodeIndex_to_nodeName(_unitigs[repeatUnitigIndex]._startNode) << endl;
                //cout << BiGraph::nodeIndex_to_nodeName(_unitigs[sideUnitigIndexSucc]._startNode) << endl;
                //cout << BiGraph::nodeIndex_to_nodeName(_unitigs[sideUnitigIndexPred]._endNode) << endl;
                //getchar();
                //loopRepeats.push_back({loopUnitigIndex, repeatUnitigIndex, sideUnitigIndexSucc, sideUnitigIndexPred});

                //vector<u_int32_t> unitigNodes; 
                //getUnitigNodes(unitig, unitigNodes);
                for(u_int32_t node : _unitigs[loopUnitigIndex]._nodes){
                    removedNodes.insert(node);
                    removedNodes.insert(nodeIndex_toReverseDirection(node));
                    //saveState._nodeNameRemoved_tmp.insert(BiGraph::nodeIndex_to_nodeName(node));
                }

                for(u_int32_t nodeIndex : _unitigs[repeatUnitigIndex]._nodes){
                    //if(_repeatNodes.find(node) != _repeatNodes.end()) continue;
                    _repeatNodes[nodeIndex] = {_unitigs[loopUnitigIndex]._nodes, _unitigs[repeatUnitigIndex]._nodes};
                    saveState._repeatNodes[nodeIndex] = {_unitigs[loopUnitigIndex]._nodes, _unitigs[repeatUnitigIndex]._nodes};

                    _repeatNodes[nodeIndex_toReverseDirection(nodeIndex)] = {_unitigs[unitigIndex_toReverseDirection(loopUnitigIndex)]._nodes, _unitigs[unitigIndex_toReverseDirection(repeatUnitigIndex)]._nodes};
                    saveState._repeatNodes[nodeIndex_toReverseDirection(nodeIndex)] = {_unitigs[unitigIndex_toReverseDirection(loopUnitigIndex)]._nodes, _unitigs[unitigIndex_toReverseDirection(repeatUnitigIndex)]._nodes};
                    //_repeatNodes[nodeIndex_toReverseDirection(nodeIndex)] = {_unitigs[loopUnitigIndex]._nodes, _unitigs[repeatUnitigIndex]._nodes};
                    //saveState._repeatNodes[nodeIndex] = {_unitigs[loopUnitigIndex]._nodes, _unitigs[repeatUnitigIndex]._nodes};
                    //cout << _repeatNodes[nodeIndex]._loopNodes.size() << _repeatNodes[nodeIndex]._repeatNodes.size() << endl;
                    //cout << nodeIndex << endl;
                }

                removedUnitigs.insert(repeatUnitigIndex);
                removedUnitigs.insert(unitigIndex_toReverseDirection(repeatUnitigIndex));
        ...
        for(u_int32_t nodeIndex : removedNodes){
            ...
            //cout << "Remove: " << BiGraph::nodeIndex_to_nodeName(nodeIndex) << endl;
	...
	bool solveLoopRepeat_withMiddle(u_int64_t maxRepeatLength, u_int32_t loopUnitigIndex, u_int32_t& repeatUnitigIndex, u_int32_t& sideUnitigIndexSucc, u_int32_t& sideUnitigIndexPred){

        repeatUnitigIndex = -1;
        sideUnitigIndexSucc = -1;
        sideUnitigIndexPred = -1;
        ...
        getSuccessors_unitig(loopUnitigIndex, 0, loopSuccessors);
        if(loopSuccessors.size() != 1) return false;

		vector<u_int32_t> loopPredecessors;
        getPredecessors_unitig(loopUnitigIndex, 0, loopPredecessors);
        if(loopPredecessors.size() != 1) return false;

        if(loopSuccessors[0] != loopPredecessors[0]) return false;

        repeatUnitigIndex = loopSuccessors[0];

        //cout << _unitigs[repeatUnitigIndex]._length << endl;
        //if(_unitigs[repeatUnitigIndex]._length > maxRepeatLength) return false;

        for(u_int32_t nodeIndex : _unitigs[repeatUnitigIndex]._nodes){
            if(_repeatNodes.find(nodeIndex) != _repeatNodes.end()) return false;
        }

		vector<u_int32_t> repeatSuccessors;
        getSuccessors_unitig(repeatUnitigIndex, 0, repeatSuccessors);
        if(repeatSuccessors.size() != 2) return false;

		vector<u_int32_t> repeatPredecessors;
        getPredecessors_unitig(repeatUnitigIndex, 0, repeatPredecessors);
        if(repeatPredecessors.size() != 2) return false;

        //u_int32_t sideUnitigIndexSucc = -1;
        for(u_int32_t unitigIndex : repeatSuccessors){
            if(unitigIndex != loopUnitigIndex){
                sideUnitigIndexSucc = unitigIndex;
                break;
            }
        }

        //u_int32_t sideUnitigIndexPred = -1;
        for(u_int32_t unitigIndex : repeatPredecessors){
            if(unitigIndex != loopUnitigIndex){
                sideUnitigIndexPred = unitigIndex;
                break;
            }
        }


        //if(_unitigs[sideUnitigIndexPred]._length < 100000) return false;
        //if(_unitigs[sideUnitigIndexSucc]._length < 100000) return false;

        //cout << loopUnitigIndex << " " << repeatUnitigIndex << " " << sideUnitigIndexSucc << " " << sideUnitigIndexPred << endl;
        //cout << BiGraph::nodeIndex_to_nodeName(_unitigs[repeatUnitigIndex]._startNode) << endl;

		vector<u_int32_t> sidePredecessors;
        getPredecessors_unitig(sideUnitigIndexSucc, 0, sidePredecessors);

		vector<u_int32_t> sideSuccessors;
        getSuccessors_unitig(sideUnitigIndexPred, 0, sideSuccessors);

        //cout << sidePredecessors.size() << " " << sideSuccessors.size() << endl;
        if(sidePredecessors.size() != 1) return false;
        if(sideSuccessors.size() != 1) return false;


        if(sideUnitigIndexSucc != sideUnitigIndexPred) return false;
        if(_unitigs[sideUnitigIndexSucc]._length + _unitigs[loopUnitigIndex]._length < 500000) return false;

        return true;
	}

    struct LoopRepeatWithoutMiddleMemo{
        u_int32_t _loopUnitigIndex;
        //u_int32_t _repeatUnitigIndex;
        u_int32_t _sideUnitigIndexSucc;
        u_int32_t _sideUnitigIndexPred;
    ...
    u_int64_t removeLoopRepeat_withoutMiddle(float maxRepeatLength, SaveState2& saveState){

        vector<LoopRepeatWithoutMiddleMemo> loopRepeats;
        unordered_set<u_int32_t> removedNodes;
        unordered_set<u_int32_t> removedUnitigs;
        ...
        for(const UnitigTip& unitigTip : unitigTips){
	        ...
	        bool isRepeat = solveLoopRepeat_withoutMiddle(maxRepeatLength, loopUnitigIndex, repeatUnitigIndex, sideUnitigIndexSucc, sideUnitigIndexPred);
            //cout << isRepeat << endl;
            if(isRepeat){

                cout << "Found repeat without middle!" << endl;
                loopRepeats.push_back({loopUnitigIndex, sideUnitigIndexSucc, sideUnitigIndexPred});
        ...
        for(const LoopRepeatWithoutMiddleMemo& repeat : loopRepeats){
            const Unitig& unitigLoop = _unitigs[repeat._loopUnitigIndex];
            const Unitig& unitigSucc = _unitigs[repeat._sideUnitigIndexSucc];
            const Unitig& unitigPred = _unitigs[repeat._sideUnitigIndexPred];

            saveState._isEdgeRemoved.push_back({unitigPred._endNode, unitigSucc._startNode});
            _graphSuccessors->setEdgeRemoved(unitigPred._endNode, unitigSucc._startNode, true);

            saveState._isEdgeRemoved.push_back({unitigLoop._endNode, unitigLoop._startNode});
            _graphSuccessors->setEdgeRemoved(unitigLoop._endNode, unitigLoop._startNode, true);


            removedUnitigs.insert(unitigSucc._index);
            removedUnitigs.insert(unitigPred._index);
            removedUnitigs.insert(unitigLoop._index);
	...
	bool solveLoopRepeat_withoutMiddle(u_int64_t maxRepeatLength, u_int32_t loopUnitigIndex, u_int32_t& repeatUnitigIndex, u_int32_t& sideUnitigIndexSucc, u_int32_t& sideUnitigIndexPred){
		...
		vector<u_int32_t> loopSuccessors;
        getSuccessors_unitig(loopUnitigIndex, 0, loopSuccessors);
        if(loopSuccessors.size() != 2) return false;

		vector<u_int32_t> loopPredecessors;
        getPredecessors_unitig(loopUnitigIndex, 0, loopPredecessors);
        if(loopPredecessors.size() != 2) return false;
        ...
        isLoop = false;
        for(u_int32_t unitigIndex : loopPredecessors){
            if(unitigIndex == loopUnitigIndex){
                isLoop = true;
            }
            else{
        ...
        //cout << sidePredecessors.size() << " " << sideSuccessors.size() << endl;
        if(sidePredecessors.size() != 2) return false;
        if(sideSuccessors.size() != 2) return false;

        bool isLoopRepeat = false;
        if((sideSuccessors[0] == loopUnitigIndex && sideSuccessors[1] == sideUnitigIndexSucc) || sideSuccessors[1] == loopUnitigIndex && sideSuccessors[0] == sideUnitigIndexSucc){
            if((sidePredecessors[0] == loopUnitigIndex && sidePredecessors[1] == sideUnitigIndexPred) || sidePredecessors[1] == loopUnitigIndex && sidePredecessors[0] == sideUnitigIndexPred){
                isLoopRepeat = true;
            }
        }
    ...
    */
    ...
    void compact(bool rebuild, const vector<UnitigData>& unitigDatas){
        ...
        if(rebuild){
            ...
            for(u_int32_t unitigIndex : _removedUnitigs){
                ...
                for(u_int32_t nodeIndex : _unitigs[unitigIndex]._nodes){
                    ...
                    if(_repeatNodes.find(nodeIndex) != _repeatNodes.end()) continue;
                    //if(_isNodeValid2.find(nodeIndex) == _isNodeValid2.end()) continue;
            ...
            for(size_t nodeIndex=0; nodeIndex<_graphSuccessors->_nbNodes; nodeIndex++){
                if(!_isNodeRemoved[nodeIndex] && (_repeatNodes.find(nodeIndex) == _repeatNodes.end())){
                    nodes.push_back(nodeIndex);
                }
                //for (u_int32_t nodeIndex : _isNodeValid2){
            }
        ...
        */
        ...
        /*
        cout << _isNodeRemoved.size() <<endl;
        cout << _isNodeRemoved[11471] << " " << _nodeToUnitig[11471] << endl;
        cout << _isNodeRemoved[18000] << endl;
        cout << _isNodeRemoved[4806] << endl;
        cout << "allo " <<  _isNodeRemoved[10070] << endl;
        
        cout << (_repeatNodes.find(11471) != _repeatNodes.end()) << " " << (_repeatNodes.find(18000) != _repeatNodes.end()) << endl;
        
        if(!_isNodeRemoved[11471]){
            cout << "RAH: " << nodeIndex_to_unitig(11471)._length << " " << nodeIndex_to_unitig(11471)._abundance << endl;
        }
        */
	...
	class UnitigFunctor {
		...
		void operator () (u_int32_t nodeIndex) {
            ...
            while(true){
                ...
                //cout << (_graph->_repeatNodes.find(node) != _graph->_repeatNodes.end()) << endl;
                //cout << "access repeat nodes" << endl;
                if(_graph->_repeatNodes.find(node) != _graph->_repeatNodes.end()){
                    ...
                    if(!isInRepeat){
                        ...
                        cout << "Add repeat region"<< endl;
                        //cout << node << endl;
                        LoopRepeat repeat = _graph->_repeatNodes[node];

                        //#pragma omp critical(accessRepeatNodes)
                        //{
                        //    repeat = _graph->_repeatNodes[nodeIndex];
                        //}
                        
                        //cout << "Found repeat lala!" << endl;
                        //getchar();
                        //cout << repeat._repeatNodes.size() << endl;
                        //cout << repeat._loopNodes.size() << endl;
                        isInRepeat = true;
                        for(u_int32_t nodeIndex : repeat._repeatNodes){
                            nodes.push_back(nodeIndex);
                            nodesRC.push_back(nodeIndex_toReverseDirection(nodeIndex));
                            nbNodes += 1;
                            ...
                            if(_graph->nodeIndex_toReverseDirection(nodeIndex) == 11471){
                                cout << "RORORO" << endl;
                            }
                        }
                        for(u_int32_t nodeIndex : repeat._loopNodes){
                            nodes.push_back(nodeIndex);
                            nodesRC.push_back(nodeIndex_toReverseDirection(nodeIndex));
                            nbNodes += 1;
                        }
            ...
            if(hasRepeat){
                ...
                #pragma omp critical(unitig)
                {
                	...
                	ofstream colorFile(_graph->_outputDir + "/color.csv");
		            colorFile << "Name,Color" << endl;

                    vector<bool> memo = _graph->_isNodeRemoved;

                    _graph->_isNodeRemoved = vector<bool>(_graph->_isNodeRemoved.size(), false);
                    
                    for(size_t i=0; i<nodes.size()-1; i++){
                        u_int32_t nodeIndex = nodes[i];
                        cout << BiGraph::nodeIndex_to_nodeName(nodeIndex) << " " << BiGraph::nodeIndex_to_nodeName(nodes[i+1]) << endl;
                        
                        vector<u_int32_t> successors;
                        _graph->getSuccessors(nodeIndex, 0, successors);

                        colorFile << BiGraph::nodeIndex_to_nodeName(nodeIndex) << ",red" << endl;
                        if(std::find(successors.begin(), successors.end(), nodes[i+1]) == successors.end()){
    ...
    */
    ...
    void getSuccessors_unitig(u_int32_t unitigIndex, float abundanceCutoff, vector<u_int32_t>& successors){
        ...
        for(u_int32_t nodeIndex : successors_nodeIndex){
            ...
            //cout << "succ:" << nodeIndex << " " << _nodeToUnitig[nodeIndex]  << endl;
            //cout <<BiGraph::nodeIndex_to_nodeName(nodeIndex) << " " << _nodeToUnitig[nodeIndex] << endl;
            //cout << (_repeatNodes.find(nodeIndex) != _repeatNodes.end()) << endl;
            //cout << nodeIndex << " " <<  << endl;
            //_logFile << nodeIndex << " " << _nodeToUnitig[nodeIndex] << endl;
    ...
    void getPredecessors_unitig(u_int32_t unitigIndex, float abundanceCutoff, vector<u_int32_t>& predecessors){
        ...
        for(u_int32_t nodeIndex : predecessors_nodeIndex){
            ...
            //cout << "pred:" << nodeIndex << " " << _nodeToUnitig[nodeIndex] << endl;
            //_logFile << nodeIndex << " " << _nodeToUnitig[nodeIndex] << endl;
            //if(_nodeToUnitig.find(nodeIndex) == _nodeToUnitig.end()) continue; //reinserting bubble
            //if(_isNodeInvalid_tmp.find(nodeIndex_to_unitigIndex(nodeIndex)) != _isNodeInvalid_tmp.end()) continue;
            //if(_isUnitigInvalid_tmp.find(nodeIndex_to_unitigIndex(nodeIndex)) != _isUnitigInvalid_tmp.end()) continue;
    ...
    bool isSameUnitig(u_int32_t unitigIndex1, u_int32_t unitigIndex2){
        if(unitigIndex1 == unitigIndex2) return true;

        u_int32_t startNodeIndex = _unitigs[unitigIndex1]._startNode;
        u_int32_t endNodeIndex = _unitigs[unitigIndex2]._startNode;

        if(startNodeIndex == nodeIndex_toReverseDirection(endNodeIndex)) return true;
        ...
        while(true){
            ...
            while(true){
                ...
                while(true){
                    ...
                    u_int64_t nbSelfLoopRemoved = removeLoopRepeat_withoutMiddle(500000, currentSaveState);
                ...
                while(true){
                    ...
                    u_int64_t nbSelfLoopRemoved = removeLoopRepeat_withMiddle(500000, currentSaveState);
        ...
        if(!saveStateExist){
            ...
            */
            ...
            for(u_int32_t nodeName : currentSaveState._nodeNameRemoved_tmp){
                if(_isNodeNameRemovedAndSaved[nodeName]) continue; //Prevent nodes in repeat to be removed multiple time (repeat hack)
                //if(_isNodeRemoved[BiGraph::nodeName_to_nodeIndex(nodeName, true)]) continue;
                //if(savedRemovedNodeNames.find(nodeName) != savedRemovedNodeNames.end()) continue;
                _isNodeNameRemovedAndSaved[nodeName] = true;
                //savedRemovedNodeNames.insert(nodeName);
            ...
            */
            //currentSaveState._repeatNodes = _repeatNodes;
