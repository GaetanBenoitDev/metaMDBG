...
Illumina:
./bin/metaMDBG readCorrection ~/appa/run/correction/nanopore_AD_circ1_asm/tmp/  --illumina ~/appa/data/nanopore/subreads/circ1_illumina.fastq -t 32
*/
...
public:
	...
	typedef phmap::parallel_flat_hash_map<u_int64_t, vector<u_int64_t>, phmap::priv::hash_default_hash<u_int64_t>, phmap::priv::hash_default_eq<u_int64_t>, std::allocator<std::pair<u_int64_t, vector<u_int64_t>>>, 4, std::mutex> ReadToMinimizersMap;
    ...
    void execute (){
		...
		if(_illuminaFilename != ""){
			...
			cout << "Mapping illumina short reads" << endl;
			mapIlluminaReads();

			cout << "Loading illumina mapping" << endl;
			loadIlluminaMapping();
		}
		*/
		...
		_debug_outputFile_nbMinimizerPairs = ofstream(_inputDir + "/debug_nbMinimizerPairs.tsv");
		_debug_outputFile_nbMinimizerPairs << "AlignmentLength\tMinimizerPairs" << endl;
		cout << "Indexing alignment" << endl;
		indexReadName(_nanoporeFilename);
		PafParser pafParser(_pafFilename);
		auto fp = std::bind(&ReadCorrection::experiment_parseAlignmentsGz_nbPairs, this, std::placeholders::_1);
		pafParser.parse(fp);
		*/
	...
	/*
	void mapIlluminaReads(){


		if(fs::exists(_illuminaToNanoporeMappingFilename)){
			cout << "Skip Illumina mapping exists: " << _illuminaToNanoporeMappingFilename << endl;
			return;
		}

		ofstream dummyFile(_illuminaToNanoporeMappingFilename);
		dummyFile.close();
		


		ReadParser parser(_illuminaFilename, false, false, _logFile);
		...
		for(size_t i=0; i<parser._filenames.size(); i+=2){
			...
			cout << "Mapping illumina reads: " << filename1 << " " << filename2 << endl;

			string command = "minimap2 -t " + to_string(_nbCores) + " -x sr " + _nanoporeFilename + " " + filename1 + " " + filename2 + " >> " + _illuminaToNanoporeMappingFilename;
	...
	void loadIlluminaMapping(){


		cout << "Indexing read name nanopore" << endl;
		indexReadName(_nanoporeFilename);

		cout << "Indexing read name illumina" << endl;
		ReadParser parser(_illuminaFilename, false, false, _logFile);
		...
		for(size_t i=0; i<parser._filenames.size(); i+=2){
			...
			cout << "\tIndexing read name illumina: " << filename1 << endl;
			indexReadNameIllumina(filename1);
		}


		cout << "Index illumina to nanopore mapping" << endl;
		PafParser pafParser(_illuminaToNanoporeMappingFilename);
		auto fp = std::bind(&ReadCorrection::loadIlluminaMapping_read, this, std::placeholders::_1);
		pafParser.parse(fp);

		_illumina_readName_to_readIndex.clear();
		_readName_to_readIndex.clear();
		cout << "Clear nanopore read name idnex si plus en debug" << endl;

		cout << "Indexing illumina minimizers" << endl;
		indexIlliminaMinimizers();
	}

	void indexReadNameIllumina(const string& filename){
		auto fp = std::bind(&ReadCorrection::indexReadNameIllumina_read, this, std::placeholders::_1);
		ReadParser readParser(filename, true, false, _logFile);
		readParser.parse(fp);
	}

	void indexReadNameIllumina_read(const Read& read){
		_illumina_readName_to_readIndex[Utils::shortenHeader(read._header)] = read._index;
	}

	void loadIlluminaMapping_read(const string& line){

		//cout << line << endl;
		GfaParser::tokenize(line, _fields, '\t');

		const string& readNameIllumina = Utils::shortenHeader((*_fields)[0]);
		const string& readNameNanopore = Utils::shortenHeader((*_fields)[5]);

		if(_illumina_readName_to_readIndex.find(readNameIllumina) == _illumina_readName_to_readIndex.end()) return;
		if(_readName_to_readIndex.find(readNameNanopore) == _readName_to_readIndex.end()) return;
		//if(_readName_to_readIndex.find(readNameHiFi) == _readName_to_readIndex.end()) return;

		//cout << readNameIllumina << " " << readNameNanopore << endl;
		//cout << (_illumina_readName_to_readIndex.find(readNameIllumina) != _illumina_readName_to_readIndex.end()) << endl;
		//cout << (_readName_to_readIndex.find(readNameNanopore) != _readName_to_readIndex.end()) << endl;

		u_int64_t readIndexIllumina = _illumina_readName_to_readIndex[readNameIllumina];
		u_int64_t readIndexNanopore = _readName_to_readIndex[readNameNanopore];

		
		double readLength = stoull((*_fields)[1]);
		double readStart = stoull((*_fields)[2]);
		double readEnd = stoull((*_fields)[3]);
		
		if((readEnd-readStart)/readLength < 0.5) return;

		vector<u_int64_t>& illuminaIndexes = _nanoporeIndex_to_illuminaIndexes[readIndexNanopore];

		if(std::find(illuminaIndexes.begin(), illuminaIndexes.end(), readIndexIllumina) != illuminaIndexes.end()) return;

		illuminaIndexes.push_back(readIndexIllumina);

	}
	
	void indexIlliminaMinimizers(){
		...
		if(fs::exists(_inputDir + "/read_data_illumina.txt")){
			...
			string command = _filename_exe + " readSelectionPaired " + _inputDir + " " + _inputDir + "/read_data_illumina.txt" + " " + _illuminaFilename + " -t " + to_string(_nbCores);
			Utils::executeCommand(command, _inputDir, _logFile);
		}

		cout << "Indexing illumina minimizers" << endl;
		KminmerParserParallel parser(_inputDir + "/read_data_illumina.txt", _minimizerSize, _kminmerSize, false, false, _nbCores);
		parser.parseSequences(IndexIlluminaMinimizersFunctor(*this));

	}

	class IndexIlluminaMinimizersFunctor {

		public:

		ReadCorrection& _parent;

		IndexIlluminaMinimizersFunctor(ReadCorrection& parent) : _parent(parent){
		}

		IndexIlluminaMinimizersFunctor(const IndexIlluminaMinimizersFunctor& copy) : _parent(copy._parent){
		}

		~IndexIlluminaMinimizersFunctor(){
		}
		...
		void operator () (const KminmerList& kminmerList) {
			...
			if(readIndex % 100000 == 0) cout << readIndex << " " << _parent._illuminaIndex_to_minimizers.size() << endl;

			for(u_int64_t minimizer : kminmerList._readMinimizers){


				_parent._illuminaIndex_to_minimizers.lazy_emplace_l(readIndex,
				[&minimizer](ReadToMinimizersMap::value_type& v) { // key exist
					if(std::find(v.second.begin(), v.second.end(), minimizer) == v.second.end()){
						v.second.push_back(minimizer);
					}
				},           
				[&minimizer, &readIndex](const ReadToMinimizersMap::constructor& ctor) { // key inserted
					
					vector<u_int64_t> minimizers = {minimizer};

					ctor(readIndex, minimizers); 
	...
	*/
	...
	phmap::parallel_flat_hash_map<string, u_int32_t> _illumina_readName_to_readIndex;
	...
	unordered_map<u_int32_t, unordered_map<u_int32_t, u_int32_t>> _readIndex_to_readMappings;

	void experiment_parseAlignmentsGz_nbPairs(const string& line){

		//cout << line << endl;
		GfaParser::tokenize(line, _fields, '\t');

		const string& readNameNanopore1 = Utils::shortenHeader((*_fields)[5]);
		const string& readNameNanopore2 = Utils::shortenHeader((*_fields)[0]);

		//cout << (_readName_to_readIndex.find(readNameNanopore) != _readName_to_readIndex.end()) << endl;
		//cout << (_readName_to_readIndex.find(readNameHiFi) != _readName_to_readIndex.end()) << endl;

		if(_readName_to_readIndex.find(readNameNanopore1) == _readName_to_readIndex.end()) return;
		if(_readName_to_readIndex.find(readNameNanopore2) == _readName_to_readIndex.end()) return;

		u_int64_t readIndexNanopore1 = _readName_to_readIndex[readNameNanopore1];
		u_int64_t readIndexNanopore2 = _readName_to_readIndex[readNameNanopore2];


		//if(readIndex1 == readIndex2) return;

		double nbMatches = stoull((*_fields)[9]);
		u_int64_t alignLength = stoull((*_fields)[10]);

		float identity = nbMatches / alignLength;

		if(identity < 0.98) return;
		
		_readIndex_to_readMappings[readIndexNanopore1][readIndexNanopore2] = alignLength;
		/*
		u_int32_t readStart = stoull((*_fields)[2]);
		u_int32_t readEnd = stoull((*_fields)[3]);
		u_int32_t contigLength = stoull((*_fields)[6]);
	...
	//"reprise: on veut essayer de mapper les contigs sur le graph k=4 ou 5, puis combiner les contig via les read path"
	//"reprise: verifier qu'on eprd pas les long reads, subreads: il doit manquer des reads car metaflye tres fragmenter par rapport aur esultat final"
	//"revoir le truc de rayan: beyond homopolimer compressed truc"
	//"output les graph a chaque abundance filter step pour voir un peu a quoi ressemble le graph k=4, voir si on peut extend les unitigs"
	...
	*/
	...
	class LoadMinimizerReadsFunctor {
		...
		void operator () (const KminmerList& kminmerList) {
			...
			if(_loadHifi){
				_parent._mReadsHiFi.push_back({kminmerList._readIndex, kminmerList._readMinimizers, kminmerList._readQualities});
			}
			else{
				_parent._mReads.push_back({kminmerList._readIndex, kminmerList._readMinimizers, kminmerList._readQualities});
			}
	...
	*/
	...
	void correctReads(){
		...
		KminmerParserParallel parser2(_inputDir + "/read_data_init.txt", _minimizerSize, _kminmerSize, false, true, _nbCores);
	...
	void correctRead(const KminmerList& kminmerList, const std::unique_ptr<spoa64::AlignmentEngine>& al, const std::unique_ptr<spoa64::AlignmentEngine>& alTrimming){
		...
		vector<u_int64_t> correctedReadMinimizers = correctWindow(0, readIndex, kminmerList._readMinimizers, kminmerList._readQualities, _print_debug, al, alTrimming);
	...
	vector<u_int64_t> correctWindow(size_t windowIndex, u_int64_t readIndex, const vector<u_int64_t>& readMinimizers, const vector<u_int8_t>& readQualities, bool print_debug, const std::unique_ptr<spoa64::AlignmentEngine>& al, const std::unique_ptr<spoa64::AlignmentEngine>& alTrimming){
		...
		for(const auto& it : readIndex_to_matchCount){
			...
			//Experiment nb pairs vs mapping
			if(nbMatches > 0){
				u_int32_t readIndex1 = readIndex;
				u_int32_t readIndex2 = it.first;

                DbgEdge edge = {readIndex1, readIndex2};
				edge = edge.normalize();
				...
				#pragma omp critical
				{
					if(_isReadPairWritten.find(edge) == _isReadPairWritten.end()){
						isWritten = false;
						_isReadPairWritten.insert(edge);
					}
				}


				if(!isWritten){
					u_int32_t alignmentLength = 0;

					if(_readIndex_to_readMappings.find(readIndex1) != _readIndex_to_readMappings.end()){
						if(_readIndex_to_readMappings[readIndex1].find(readIndex2) != _readIndex_to_readMappings[readIndex1].end()){
							alignmentLength = _readIndex_to_readMappings[readIndex1][readIndex2];
						}
					}

					if(_readIndex_to_readMappings.find(readIndex2) != _readIndex_to_readMappings.end()){
						if(_readIndex_to_readMappings[readIndex2].find(readIndex1) != _readIndex_to_readMappings[readIndex2].end()){
							if(_readIndex_to_readMappings[readIndex2][readIndex1] > alignmentLength){
								alignmentLength = _readIndex_to_readMappings[readIndex2][readIndex1];
							}
						}
					}


					if(alignmentLength > 2500){
					...
					if(alignmentLength > 0){
						...
						#pragma omp critical
						{
							_debug_outputFile_nbMinimizerPairs << alignmentLength << "\t" << nbMatches << endl;
						}
					}
		...
		for(u_int32_t rIndex : matchingReadIndexes){
			...
			//if(computeSimilarity(minimizerSet, matchingReadMinimizers) < 0.5) continue;
		...
		*/
		...
		vector<MinimizerRead> mappedReads;
		getMappedMinimizerReads(minimizerSequences, readMinimizers, mappedReads, al);


		if(print_debug) cout << "Nb mapped mReads: " << mappedReads.size() << endl;
		

		double nbAlignedMinimizers = 0;
		for(size_t i=0; i<mappedReads.size(); i++){
			nbAlignedMinimizers += mappedReads[i]._minimizers.size();
		}


		//if(mappedReads.size() < 5) return readMinimizers;
		float alignCoverage = nbAlignedMinimizers / readMinimizers.size();
		//if(alignCoverage < 10) return readMinimizers;


		unordered_set<u_int64_t> illuminaMinimizers;
		collectIlluminaMinimizers(mappedReads, illuminaMinimizers);
		...
		MinimizerRead minimizerReadCorr = applyIlluminaCorrection({0, readMinimizers, readQualities}, illuminaMinimizers);

		vector<u_int64_t> weights(minimizerReadCorr._qualities.size(), 1);
		...
		for(size_t i=0; i<mappedReads.size(); i++){
			...
			spoa64::Alignment alignment = al->Align(
							minimizerReadIlluminaCorrected._minimizers,
							minimizerReadIlluminaCorrected._minimizers.size(),
							graph);

			vector<u_int64_t> qualities(minimizerReadIlluminaCorrected._qualities.size(), 0);
			for(size_t i=0; i<minimizerReadIlluminaCorrected._qualities.size(); i++){
				qualities[i] = minimizerReadIlluminaCorrected._qualities[i];
			}


			alignments.push_back({minimizerReadIlluminaCorrected._minimizers, qualities, alignment, 0});
		...
		for(size_t i=0; i<alignments.size(); i++){
			...
			if(print_debug){
				...
				for(u_int64_t m : alignment._minimizerSequence){
					//cout << "\t" << m << endl; 
				}
		...
		if(_print_debug){
			cout << "Nb illumina minimizers: " << illuminaMinimizers.size() << endl;
		}
		...
		*/
		...
		vector<u_int64_t> correctedReadMinimizers = performCorrection(graph, readMinimizers, al, alTrimming, correctedCoverages, minimizerSet);
		...
		*/
		...
		if(print_debug){
			...
			cout << readMinimizers.size() << " " << correctedReadMinimizers.size() << endl;
			...
			if(_hifiFilename != ""){
				...
				if(_nanoporeReadIndex_to_bestHifiMinimizerRead.find(readIndex) != _nanoporeReadIndex_to_bestHifiMinimizerRead.end()){
					...
					*/
					...
					/*
					cout << "Nanopore read:" << endl;
					for(const auto& it : _readName_to_readIndex){
						if(it.second == readIndex){
							cout << it.first << endl;
							//break;
						}
					}
					...
					cout << "HiFi read:" << endl;
					for(const auto& it : _readName_to_readIndex){
						if(it.second == _nanoporeReadIndex_to_bestHifiMinimizerRead[readIndex]._readIndex){
							cout << it.first << endl;
							//break;
						}
					}
					*/
					...
					savePoaGraph(graph, _inputDir + "/poaGraph.gfa", readNodes, illuminaMinimizers);
	...
	void getMappedMinimizerReads(const vector<MinimizerRead>& minimizerSequences, const vector<u_int64_t>& readMinimizers, vector<MinimizerRead>& mappedReads, const std::unique_ptr<spoa64::AlignmentEngine>& al){
		...
		//getchar();
		/*
      	//graph.coder_.resize(_progressiveAbundanceFilter->_unitigGraph->_nodes.size(), -1);
      	//graph.decoder_.resize(_progressiveAbundanceFilter->_unitigGraph->_nodes.size(), -1);
		  
		//minimizerSequences.insert(minimizerSequences.begin(), kminmerList._readMinimizers);
		...
		*/
		...
		for(size_t i=0; i<minimizerSequences.size(); i++){
			...
			vector<u_int8_t> qualities = mOverlap._qualities;
			...
			int lastMatchPos = 0;
			double alignmentScore = getAlignmentScore(readMinimizers, minimizerSequence, al, lastMatchPos) / (double) readMinimizers.size();// / (double) minimizerSequence.size();
	...
	void collectIlluminaMinimizers(const vector<MinimizerRead>& nanoporeReads, unordered_set<u_int64_t>& illuminaMinimizers){

		illuminaMinimizers.clear();

		for(const MinimizerRead& minimizerRead : nanoporeReads){

			u_int64_t nanoporeReadIndex = minimizerRead._readIndex;

			if(_nanoporeIndex_to_illuminaIndexes.find(nanoporeReadIndex) == _nanoporeIndex_to_illuminaIndexes.end()) continue;

			for(u_int64_t illuminaReadIndex : _nanoporeIndex_to_illuminaIndexes[nanoporeReadIndex]){

				if(_illuminaIndex_to_minimizers.find(illuminaReadIndex) == _illuminaIndex_to_minimizers.end()) continue;

				for(u_int64_t minimizer : _illuminaIndex_to_minimizers[illuminaReadIndex]){
					illuminaMinimizers.insert(minimizer);
				}
	...
	MinimizerRead applyIlluminaCorrection(const MinimizerRead& nanoporeRead, unordered_set<u_int64_t>& illuminaMinimizers){

		if(_illuminaFilename.size() == 0) return nanoporeRead;
		vector<u_int64_t> minimizers;
		vector<u_int8_t> qualities;

		for(size_t i=0; i<nanoporeRead._minimizers.size(); i++){

			u_int64_t minimizer = nanoporeRead._minimizers[i];
			u_int8_t quality = nanoporeRead._qualities[i];

			if(illuminaMinimizers.find(minimizer) == illuminaMinimizers.end()) continue;

			minimizers.push_back(minimizer);
			qualities.push_back(quality);
		}

		return {nanoporeRead._readIndex, minimizers, qualities};
	}
	...
	*/
	...
	vector<u_int64_t> performCorrection(spoa64::Graph& poaGraph, const vector<u_int64_t>& readMinimizers, const std::unique_ptr<spoa64::AlignmentEngine>& al, const std::unique_ptr<spoa64::AlignmentEngine>& alTrimming, vector<u_int64_t>& correctedCoverages, unordered_set<u_int64_t>& minimizerSet){
		...
		*/
		...
		/*
		spoa64::Alignment consensusAlignment = al->Align(readMinimizers, readMinimizers.size(), poaGraph);
		
		u_int64_t maxPathSize = readMinimizers.size() * 2;
		u_int64_t startNode;
		u_int64_t endNode;
		poaGraph.performCorrection(consensusAlignment, readMinimizers, readMinimizers.size(), startNode, endNode);
		*/
	
		u_int64_t maxPathSize = readMinimizers.size() * 2;
		/*
		u_int64_t startNodeId = -1;
		u_int64_t endNodeId = -1;

		for (const auto& it : poaGraph.nodes()) {

			u_int64_t minimizer = poaGraph.decoder(it->code);
			if(minimizerSet.find(minimizer) == minimizerSet.end()) continue;

			if(it->Coverage() <= 1) continue;

			if(startNodeId == -1) startNodeId = it->id;
			
			endNodeId = it->id;
			
		}

		if(startNodeId == -1) return readMinimizers;
		*/
		//cout << (startNode ) << " " << (endNode ) << endl;
		vector<u_int64_t> correctReadMinimizers2 = computePath(poaGraph, poaGraph.nodes_[0].get(), nullptr, maxPathSize, minimizerSet);
		...
		vector<u_int64_t> correctReadMinimizersTrimmed = trimCorrectedPath(readMinimizers, correctReadMinimizers2, alTrimming);
	...
	vector<u_int64_t> trimCorrectedPath(const vector<u_int64_t>& readMinimizers, const std::vector<u_int64_t>& correctedReadMinimizers, const std::unique_ptr<spoa64::AlignmentEngine>& al){
		...
		for (const auto& it : alignment) {
			...
			if(v1 == -1){ //insert in
				...
				if(correctedReadMinimizers[v1] == readMinimizers[v2]){
	...
	*/
	...
	void savePoaGraph(const spoa64::Graph& graph, const string& outputFilename, const vector<u_int64_t>& readNodes, unordered_set<u_int64_t>& illuminaMinimizers){
		...
		for (const auto& it : graph.nodes()) {
			...
			u_int64_t minimizer = graph.decoder(it->code);
