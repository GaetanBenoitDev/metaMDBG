...
/*
- Ne pas output le premier graph (trop volumineux et erroné)
- Gfa: ne pas proposer d'output le premier graph vu qu'il n'existera plus
- PeakRam usage: enelever la dependance a /usr/time et utiliser la meme methode que metaflye/minimap2
- Mettre tous les logs dans un fichier comme metaflye
- Afficher des stats sur l'assemblage en sortie d'assembleur: nb bases (assembly size), N50
- update dependecies

- Contig érroné:
	- vérifié si on a vraiment des MAG avec des contig <1-2x, si aucune mag, alors il vaut mieux virer ces contigs (verifié sur hifi et nanopore)
	- on peut aussi enlever ces contig seulement si ils sont contenu dans un autre contig plus long et low coverage etc
*/
...
const char ARG_MIN_READ_QUALITY = 'q';
...
public: 
	...
	void execute(const char* sequence, size_t length, string& rleSequence, vector<u_int64_t>& rlePositions) {
		...
		//cout << "ENCODER RLE: Attention msr" << endl;
		rleSequence = string(sequence);
		for(size_t i=0; i<rleSequence.size(); i++){
			rlePositions.push_back(i);
		}
		return;
		//return executeMSR(sequence, length, rleSequence, rlePositions);
...
public:
	...
	ReadParserParallelPaired(const string& inputFilename1, const string& inputFilename2, int nbCores, ofstream& logFile) : _logFile(logFile){

		if(!fs::exists(inputFilename1)){
			cerr << "File not found: " << inputFilename1 << endl;
			exit(1);
		}

		if(!fs::exists(inputFilename2)){
			cerr << "File not found: " << inputFilename2 << endl;
			exit(1);
		}
	...
	void parse(const Functor& functor){
		...
		_logFile << "Parsing file: " << _inputFilename1 << " " << _inputFilename2 << endl;
...
public:
	...
	void parse(const std::function<void(vector<MinimizerType>, vector<KmerVec>, vector<ReadKminmer>, u_int8_t, u_int64_t)>& fun){
		...
		while(true){
			...
			if(_usePos) file_readData.read((char*)&minimizerPos[0], (size+1)*sizeof(u_int32_t));
	...
	void parseMinspace(const std::function<void(vector<MinimizerType>, vector<ReadKminmerComplete>, u_int8_t, u_int64_t)>& fun){
		...
		while(true){
			...
			file_readData.read((char*)&minimizers[0], size*sizeof(MinimizerType));
			if(_usePos){
				file_readData.read((char*)&minimizerPos[0], (size+1)*sizeof(u_int32_t));
			}
			//if(_hasQuality) file_readData.read((char*)&minimizerQualities[0], size*sizeof(u_int8_t));
...
public:
	...
	void execute(const ChuckFunctor& functor){
		...
		while(true){
			...
			file_readData.read((char*)&minimizers[0], size*sizeof(MinimizerType));
			if(_hasQuality) file_readData.read((char*)&minimizerPos[0], (size+1)*sizeof(u_int32_t));
			...
			reads.push_back({readIndex, minimizers, minimizerPos, minimizerQualities, minimizerDirections});
	...
	template<typename Functor>
	void processChunk(const Functor& functor, const vector<MinimizerRead>& reads){
		functor(reads);
	}
