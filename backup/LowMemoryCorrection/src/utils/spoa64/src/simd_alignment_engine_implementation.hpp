...
constexpr std::uint64_t kRegisterSize = 256;
...
struct InstructionSet<A, std::int16_t> {
  ...
  static constexpr std::uint64_t kNumVar = kRegisterSize / (8 * sizeof(type));
  static constexpr std::uint64_t kLogNumVar = 4;
  static constexpr std::uint64_t kLSS = 2;   // Left Shift Size
  static constexpr std::uint64_t kRSS = 30;  // Right Shift Size
...
template<Architecture A>
struct InstructionSet<A, std::int64_t> {
  using type = std::int64_t;
  static constexpr std::uint64_t kNumVar = kRegisterSize / (8 * sizeof(type));
  static constexpr std::uint64_t kLogNumVar = 3;
  static constexpr std::uint64_t kLSS = 4;
  static constexpr std::uint64_t kRSS = 28;
  static inline __mxxxi _mmxxx_add_epi(const __mxxxi& a, const __mxxxi& b) {
...
constexpr std::uint64_t kRegisterSize = 128;
...
struct InstructionSet<A, std::int16_t> {
  ...
  static constexpr std::uint64_t kNumVar = kRegisterSize / (8 * sizeof(type));
  static constexpr std::uint64_t kLogNumVar = 3;
  static constexpr std::uint64_t kLSS = 2;
  static constexpr std::uint64_t kRSS = 14;
  static inline __mxxxi _mmxxx_add_epi(const __mxxxi& a, const __mxxxi& b) {
...
struct InstructionSet<A, std::int64_t> {
  using type = std::int64_t;
  static constexpr std::uint64_t kNumVar = kRegisterSize / (8 * sizeof(type));
  static constexpr std::uint64_t kLogNumVar = 2;
  static constexpr std::uint64_t kLSS = 4;
  static constexpr std::uint64_t kRSS = 12;
  static inline __mxxxi _mmxxx_add_epi(const __mxxxi& a, const __mxxxi& b) {
...
void _mmxxx_print(const __mxxxi& a) {
  ...
  for (std::uint64_t i = 0; i < T::kNumVar; i++) {
    std::cout << unpacked[i] << " ";
  }
}
...
template<Architecture A, typename T>
typename T::type _mmxxx_value_at(const __mxxxi& a, std::uint64_t i) {
  __attribute__((aligned(kRegisterSize / 8))) typename T::type unpacked[T::kNumVar]; // NOLINT
  _mmxxx_store_si(reinterpret_cast<__mxxxi*>(unpacked), a);
...
template<Architecture A, typename T>
std::int64_t _mmxxx_index_of(
    const __mxxxi* row,
    std::uint64_t row_width,
    typename T::type value) {
  for (std::uint64_t i = 0; i < row_width; ++i) {
    __attribute__((aligned(kRegisterSize / 8))) typename T::type unpacked[T::kNumVar]; // NOLINT
    _mmxxx_store_si(reinterpret_cast<__mxxxi*>(unpacked), row[i]);

    for (std::uint64_t j = 0; j < T::kNumVar; j++) {
      if (unpacked[j] == value) {
...
struct SimdAlignmentEngine<A>::Implementation {
  ...
  std::vector<std::int64_t> first_column;
  ...
  std::uint64_t masks_size;
  __mxxxi* masks;

  std::unique_ptr<__mxxxi[]> penalties_storage;
  std::uint64_t penalties_size;
  __mxxxi* penalties;
...
void SimdAlignmentEngine<A>::Prealloc(
  ...
  if (worst_case_score < std::numeric_limits<std::int64_t>::min() + 1024) {
    return;
  } else if (worst_case_score < std::numeric_limits<std::int16_t>::min() + 1024) {  // NOLINT
    try {
      Realloc(
          (max_sequence_len / InstructionSet<A, std::int64_t>::kNumVar) + 1,
...
void SimdAlignmentEngine<A>::Initialize(
...
#if defined(__AVX2__) || defined(__SSE4_1__) || defined(SPOA_USE_SIMDE)
  std::int64_t padding_penatly = -1 * std::max(
  ...
  for (std::uint64_t i = 0; i < graph.num_codes(); ++i) {
    ...
    for (std::uint64_t j = 0; j < matrix_width; ++j) {
      for (std::uint64_t k = 0; k < T::kNumVar; ++k) {
  ...
  switch (subtype_) {
    case AlignmentSubtype::kConvex:
      for (std::uint64_t j = 0; j < matrix_width; ++j) {
        ...
        for (std::uint64_t k = 1; k < T::kNumVar; ++k) {
  ...
  switch (type_) {
    case AlignmentType::kSW:
      ...
      for (std::uint64_t i = 0; i < matrix_height; ++i) {
...
Alignment SimdAlignmentEngine<A>::Align(
  ...
  if (worst_case_score < std::numeric_limits<std::int64_t>::min() + 1024) {
    ...
    try {
      Realloc(
          std::ceil(static_cast<double>(sequence_len) / InstructionSet<A, std::int64_t>::kNumVar),  // NOLINT
          graph.nodes().size() + 1,
          graph.num_codes());
    } catch (std::bad_alloc& ba) {
      throw std::invalid_argument(
    ...
    Initialize<InstructionSet<A, std::int64_t>>(
        sequence,
        graph,
        sequence_len,
        std::ceil(static_cast<double>(sequence_len) / InstructionSet<A, std::int64_t>::kNumVar),  // NOLINT
    ...
    if (subtype_ == AlignmentSubtype::kLinear) {
      return Linear<InstructionSet<A, std::int64_t>>(sequence_len, graph, score);  // NOLINT
    } else if (subtype_ == AlignmentSubtype::kAffine) {
      return Affine<InstructionSet<A, std::int64_t>>(sequence_len, graph, score);  // NOLINT
    } else if (subtype_ == AlignmentSubtype::kConvex) {
      return Convex<InstructionSet<A, std::int64_t>>(sequence_len, graph, score);  // NOLINT
    }
  } else {
    try {
      Realloc(
...
void SimdAlignmentEngine<A>::Initialize(
  ...
  for (std::uint64_t i = 0; i < graph.num_codes(); ++i) {
    std::uint64_t c = graph.decoder(i);
    for (std::uint64_t j = 0; j < matrix_width; ++j) {
      for (std::uint64_t k = 0; k < T::kNumVar; ++k) {
        unpacked[k] = (j * T::kNumVar + k) < normal_matrix_width ?
            (c == sequence[j * T::kNumVar + k] ? m_ : n_) : padding_penatly;
...
Alignment SimdAlignmentEngine<A>::Linear(
  ...
  for (std::uint64_t i = 0, j = 0; i < T::kNumVar && j < T::kLogNumVar; ++i) {
  ...
  for (std::uint64_t i = 1; i < T::kLogNumVar; ++i) {
  ...
  std::int64_t max_i = -1;
  std::int64_t max_j = -1;
  std::uint64_t last_column_id = (normal_matrix_width - 1) % T::kNumVar;
  __mxxxi zeroes = T::_mmxxx_set1_epi(0);
  __mxxxi g = T::_mmxxx_set1_epi(g_);
  ...
  for (const auto& it : rank_to_node) {
    ...
    if (type_ == AlignmentType::kSW) {
      std::int64_t max_row_score = _mmxxx_max_value<A, T>(score);
      ...
      if (it->outedges.empty()) {
        std::int64_t max_row_score = _mmxxx_value_at<A, T>(
  ...
  // backtrack
  std::uint64_t max_num_predecessors = 1;
  for (std::uint64_t i = 0; i < static_cast<std::uint64_t>(max_i); ++i) {
    max_num_predecessors = std::max(
        max_num_predecessors,
        static_cast<std::uint64_t>(rank_to_node[i]->inedges.size()));
  }
  ...
  std::int64_t i = max_i;
  std::int64_t j = max_j;
  std::int64_t prev_i = 0, prev_j = 0;

  std::uint64_t j_div = j / T::kNumVar;
  std::uint64_t j_mod = j % T::kNumVar;
  ...
  do {
    ...
    if (j_mod == 0) {
      ...
      if (j_div > 0) {
        ...
        for (std::uint64_t p = 0; p < predecessors.size(); ++p) {
          _mmxxx_store_si(
  ...
  if (type_ == AlignmentType::kNW) {
    ...
    while (i != 0 && j == -1) {
      ...
      if (it->inedges.empty()) {
        ...
        for (const auto& jt : it->inedges) {
          std::uint64_t pred_i = pimpl_->node_id_to_rank[jt->tail->id] + 1;
...
Alignment SimdAlignmentEngine<A>::Convex(
  ...
  for (std::uint64_t i = T::kLogNumVar + 1; i < 2 * T::kLogNumVar; ++i) {
    pimpl_->penalties[i] = T::_mmxxx_add_epi(
        pimpl_->penalties[i - 1],
        pimpl_->penalties[i - 1]);
  }
