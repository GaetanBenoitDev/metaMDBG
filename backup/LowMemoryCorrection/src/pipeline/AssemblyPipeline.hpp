...
public:
	...
	//"reprise: dl le jeu AD HiFi pour avoir une sorte de reference pour corriger"
	...
	void parseArgs(int argc, char* argv[]){
		...
		args::ValueFlag<int> arg_nbWindows(parser, "", "Maximum read coverage used for contig correction (increase for better correction)", {ARG_NB_WINDOWS}, 100);
    ...
    void execute_pipeline(){
		...
		command = _filename_exe + " readSelection " + _tmpDir + " " + _tmpDir + "/read_data_init.txt" + " " + _inputFilename + " -t " + to_string(_nbCores) + " -q " + to_string(_minReadQuality);
		executeCommand(command);
		
		//fs::copy(_tmpDir + "/read_data_init.txt", _tmpDir + "/read_data_corrected.txt", fs::copy_options::overwrite_existing);
		//fs::copy(_tmpDir + "/read_data_init.txt", _tmpDir + "/read_data_init.txt.ont", fs::copy_options::overwrite_existing);

		command = _filename_exe + " readCorrection " + _tmpDir + " -t " + to_string(_nbCores);
		executeCommand(command);
	...
	void execTest(){
		...
		writeParameters(_minimizerSize, _firstK, _minimizerDensity, _firstK, _firstK, _lastK);
			...
			command = _filename_exe + " toBasespace " + " " + _tmpDir + " " + _tmpDir + "/contig_data.txt " + " " + contigFilenameCompressed + " " + _inputFilename  + " -t " + to_string(_nbCores);
			executeCommand(command);
			...
			*/
			...
			//getchar();
			command = _filename_exe + " polish " + contigFilename_uncorrected + " " + _tmpDir + " " + Commons::inputFileToFilenames(_inputFilename) + " " + " -t " + to_string(_nbCores) + " -n " + to_string(_contigPolishing_nbReadFragments) + " --metaMDBG"; //--circ
			executeCommand(command);
	...
	void dereplicate(const string& inputContigFilename, const string& outputContigFilename){
		...
		ReadParserParallel readParser(inputContigFilename, true, false, 1, _logFile);
