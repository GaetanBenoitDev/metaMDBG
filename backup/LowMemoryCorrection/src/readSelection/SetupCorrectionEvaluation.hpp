...
public:
	...
	void parseArgs(int argc, char* argv[]){
		...
		args::Positional<std::string> arg_contigInfoFilename(parser, "contigInfoFilename", "", args::Options::Required);
	...
	void indexReadName(const string& filename, bool isFile){
		...
		ReadParser readParser(filename, isFile, false, _logFile);
		readParser.parse(fp);
	}
	...
	void parseAlignmentsGz_readVsRead(const string& line){
		...
		//}
		ReadMatch match = {contigIndex, isReversed, contigStart, contigEnd, nbMatches, alignLength, divergence};
		_readIndex_to_bestReadMatch[readIndex].push_back(match);
