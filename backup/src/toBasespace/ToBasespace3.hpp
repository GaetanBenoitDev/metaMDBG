...
#ifndef MDBG_METAG_TOBASESPACE3
#define MDBG_METAG_TOBASESPACE3
...
class ToBasespace3 : public Tool{
...
public:
    ...
    void execute(){
		...
		cout << "Finding best kminmer position" << endl;
		findBestKminmerPositions();
	...
	void countNbContigs(){
		...
		parser.parseSequences(CountContigFunctor(*this));
	}


	class CountContigFunctor {

		public:

		ToBasespace3& _parent;

		CountContigFunctor(ToBasespace3& parent) : _parent(parent){
		}

		CountContigFunctor(const CountContigFunctor& copy) : _parent(copy._parent){
		}
		...
		void operator () (const KminmerList& kminmerList) {
			_parent._initialNbContigs += 1;
		}
	};
	*/

	KminmerReadMap _kminmer_to_readIndex;
	size_t _initialNbContigs;
	size_t _nbReadsPerPartition;

	"cancel ça, on a pas les position des minimizers sur les contigs: repartir de ToBasespace2 et utiliser le minimizerAligner + si possible localisation rapide de l'endroit ou map le read sur le contig (trimming du contig)"
	void mapReads(){

		cout << "ToBasespace: besoin des meme parametre que dans la correction? minReadLength etc" << endl;
		//cout << "Nb contigs: " << _initialNbContigs << endl;


		ReadMapper readMapper(_inputDir + "/read_data_init.txt", _inputFilenameContig, _inputDir, _minimizerSize, _minimizerDensity, 1000000, _nbReads, 100000, 10, 100, 3, 0.04, _nbCores);
		readMapper.execute();

		getchar();
	}

	void findBestKminmerPositions(){

		AlignmentFile* alignmentFile = new AlignmentFile(_inputDir, _initialNbContigs, _nbReadsPerPartition, 'r');

		KminmerParserParallel parser(_inputDir + "/read_data_init.txt", _minimizerSize, _kminmerSize, false, true, _nbCores);
		parser._alignmentFile = alignmentFile;
		parser.parseSequences(FindBestKminmerPositionsFunctor(*this));
		
		delete alignmentFile;
	}

	class FindBestKminmerPositionsFunctor {

		public:

		ToBasespace3& _parent;

		FindBestKminmerPositionsFunctor(ToBasespace3& parent) : _parent(parent){
		}

		FindBestKminmerPositionsFunctor(const FindBestKminmerPositionsFunctor& copy) : _parent(copy._parent){
		}

		~FindBestKminmerPositionsFunctor(){
		}
		...
		void operator () (const KminmerList& kminmerList) {
			...
			if(readIndex % 100000 == 0) cout << "Find best kminmer positions: " << readIndex << endl;

			for(const AlignmentResult2 alignment : kminmerList._alignments){

				const vector<MinimizerType>& contigMinimizers = _parent._mContigs[alignment._queryReadIndex];
				extractAlignment(alignment, readIndex, kminmerList._readMinimizers, alignment._queryReadIndex, contigMinimizers);
		...
		void extractAlignment(const AlignmentResult2& alignment, u_int32_t readIndex, const vector<MinimizerType>& readMinimizers, u_int32_t contigIndex, const vector<MinimizerType>& contigMinimizers){
			...
			for(const Utils::CigarElement& cigarElement : cigarSequence){
				...
				if(cigarElement._cigarType == Utils::CigarType::Match){
					...
					for(size_t i=0; i<cigarElement._nbOccurences; i++){
						...
						if(readMinimizers[referencePosition] == contigMinimizers[queryPosition]){ //Match
							...
							if(prevReadPosition != -1 && prevContigPosition != -1){
								...
								//	prevContigPositionNorm = contigMinimizers.size() - contigPosition - 1;
									//return;
								//}
								indexMatchingPosition(readMinimizers[prevReadPosition], readMinimizers[referencePosition], {readIndex, (u_int32_t)prevReadPosition, alignment._chainingScore}, {contigIndex, prevContigPositionNorm});
							}
							
							prevReadPosition = referencePosition;
							prevContigPosition = queryPosition;
						}
						else{ //missmatch
	...
	class IndexContigsFunctor {
		...
		IndexContigsFunctor(ToBasespace2& toBasespace) : _toBasespace(toBasespace){
		}
		...
		void operator () (const KminmerList& kminmerList) {
			...
			for(u_int32_t i=0; i<kminmerList._kminmersInfo.size(); i++){
				...
				_toBasespace._kminmer_to_readIndex.lazy_emplace_l(vec,
	...
	*/
	...
	vector<vector<MinimizerType>> _mContigs;
	...
	class LoadContigsFunctor {
		...
		LoadContigsFunctor(ToBasespace3& parent) : _parent(parent){
		}
		...
		void operator () (const KminmerList& kminmerList) {
			...
			_parent._mContigs.push_back(kminmerList._readMinimizers);
	...
	class AlignReadFunctor {
		...
		void operator () (const KminmerList& kminmerList) {
			_parent.alignRead(kminmerList, _alignmentEngine);
		}
	};



	void alignRead(const KminmerList& kminmerList, const std::unique_ptr<spoa64::AlignmentEngine>& alignmentEngine){
		...
		for(const auto& it : readIndex_to_matchCount){
			...
			contigSequences.push_back(_mContigs[it.first]);
		}
		...
		//cout << "Matching reads: " << matchingReadIndexes.size() << endl;
		...
		for(size_t i=0; i<contigSequences.size(); i++){

			const MinimizerContig& mContig = contigSequences[i];
			...
			spoa64::Alignment alignment_forward;
			int nbMatches_forward = getAlignmentScore(graph, readMinimizers, minimizerSequence_forward, alignmentEngine, alignment_forward);
			
			spoa64::Alignment alignment_reverse;
			int nbMatches_reverse = getAlignmentScore(graph, readMinimizers, minimizerSequence_reverse, alignmentEngine, alignment_reverse);

			if(nbMatches_forward < 2 && nbMatches_reverse < 2) continue;

			if(nbMatches_forward > nbMatches_reverse){
				extractAlignment(alignment_forward, readIndex, readMinimizers, contigIndex, minimizerSequence_forward, nbMatches_forward, false);
			}
			else{
				extractAlignment(alignment_reverse, readIndex, readMinimizers, contigIndex, minimizerSequence_reverse, nbMatches_reverse, true);
			}
	...
	double getAlignmentScore(const spoa64::Graph& graph, const vector<MinimizerType>& s1, const vector<MinimizerType>& s2, const std::unique_ptr<spoa64::AlignmentEngine>& al, spoa64::Alignment& alignmentResult){
		

		spoa64::Alignment alignment = al->Align(Utils::vec32_to_vec64(s2), s2.size(), graph);
	...
	*/
	...
	class SetupKminmerSequenceExtractionFunctor {
		...
		ToBasespace3& _toBasespace;

		SetupKminmerSequenceExtractionFunctor(ToBasespace3& toBasespace) : _toBasespace(toBasespace){
		}
			...
			#pragma omp critical
			{
				...
				for(u_int32_t i=0; i<kminmerList._kminmersInfo.size(); i++){
					...
					if(_toBasespace._contigPosition_to_readPosition.find(contigPosition) == _toBasespace._contigPosition_to_readPosition.end()){
						_toBasespace._logFile << "No read position for contig position" << endl;
						continue;
					}
	...
	*/
	...
	class ExtractKminmerSequenceFunctor{
		...
		ExtractKminmerSequenceFunctor(size_t minimizerSize, float minimizerDensity, ToBasespace3& toBasespace, bool collectModel) : _toBasespace(toBasespace){
	...
	*/
	...
	void createBaseContigs(const string& contigFilename){
		...
		KminmerParser parser(contigFilename, _minimizerSize, _kminmerSize, false, false);
		auto fp = std::bind(&ToBasespace3::createBaseContigs_read, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4);
	...
	void createBaseContigs_read(const vector<MinimizerType>& readMinimizers, const vector<ReadKminmerComplete>& kminmersInfos, u_int8_t isCircular, u_int64_t readIndex){
			...
			if(i == 0){
				if(orientation){ //+
					...
					*/
					...
					//if(readMinimizers.size() > 1000) cout << i << ": " << kminmerSequence << endl;
				}
				else{
		...
		if(contigSequence.size() == 0){
			...
			for(char c : contigSequence){
				_checksum += c*contigSequence.size();
			}
