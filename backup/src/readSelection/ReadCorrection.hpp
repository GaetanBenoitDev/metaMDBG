...
public:
	...
	typedef phmap::parallel_flat_hash_map<KmerVec, vector<ReadType>, phmap::priv::hash_default_hash<KmerVec>, phmap::priv::hash_default_eq<KmerVec>, std::allocator<std::pair<KmerVec, vector<ReadType>>>, 4, std::mutex> KminmerPosMap2;
	...
	*/
	...
	void partitionReadFast(){
		...
		while(true){
			...
			cout << "Nb partitioned reads: " << _proccesedReads.size() << " / " << _totalNbReads << endl;

			_partitionReferenceReads.clear();
			_partitionRecruitedReads.clear();

			//if(_totalNbReads == _proccesedReads.size()) break;

			paritionReadFastPass();

			if(_partitionReferenceReads.size() == 0) break;

			cout << "Create partition: " << _partitionReferenceReads.size() << " " <<  _partitionRecruitedReads.size() << endl;
			for(const ReadType& readIndex : _partitionReferenceReads){
				_proccesedReads.insert(readIndex);
			}
	...
	static const u_int64_t _partitionNbReads = 5000000;
	unordered_set<ReadType> _proccesedReads;
	unordered_set<ReadType> _partitionReferenceReads;
	unordered_set<ReadType> _partitionRecruitedReads;
	unordered_set<ReadType> _tmpLala;

	void paritionReadFastPass(){

		KminmerParserParallel parser(_inputDir + "/read_data_init.txt.lowDensity", _minimizerSize, _kminmerSize, false, false, 1);
		parser.parseSequences(PartitionReadFunctor(*this));

	}



	class PartitionReadFunctor {
		...
		PartitionReadFunctor(ReadCorrection& parent) : _parent(parent){
		}

		PartitionReadFunctor(const PartitionReadFunctor& copy) : _parent(copy._parent){
		}

		~PartitionReadFunctor(){
		}
		...
		void operator () (const KminmerList& kminmerList) {
			...
			if(kminmerList._readIndex % 100000 == 0) {
				Logger::get().debug() << "\tPartition reads fast: " << Utils::getProgress(kminmerList._readIndex, _parent._totalNbReads);
				cout << kminmerList._readIndex << " " << _parent._partitionReferenceReads.size() << " " << _parent._partitionRecruitedReads.size() << endl;
			} 

			if(_parent._proccesedReads.find(kminmerList._readIndex) != _parent._proccesedReads.end()) return;

			if(_parent._partitionRecruitedReads.size() < ReadCorrection::_partitionNbReads){
				_parent._partitionReferenceReads.insert(kminmerList._readIndex);
			}

			if(_parent._partitionReferenceReads.find(kminmerList._readIndex) == _parent._partitionReferenceReads.end()) return;

			//_parent._tmpLala.insert(kminmerList._readIndex);
			//cout << kminmerList._readIndex << " " << _parent._tmpLala.size() << endl;
			...
			if(Utils::isReadTooShort(referenceRead)){
			...
			//cout << referenceRead._minimizers.size() << endl;
			//getchar();
			...
			unordered_map<ReadType, u_int32_t> queryReadNbMatches;

			for(const KmerVec& vec : uniqueReferenceKminmers){


				if(_parent._kminmer_to_readIndex2.find(vec) == _parent._kminmer_to_readIndex2.end()) continue;

				for(const ReadType& queryReadIndex : _parent._kminmer_to_readIndex2[vec]){
					//if(referenceRead._readIndex == queryReadIndex) continue;

					queryReadNbMatches[queryReadIndex] += 1;
			...
			for(const auto& it : queryReadNbMatches){

				ReadType queryReadIndex = it.first;
				if(queryReadIndex == referenceRead._readIndex) continue;


				const u_int32_t& nbMatches = it.second;
				...
				if(adding){
					if(queryReadIndex > referenceRead._readIndex){
						if(_parent._proccesedReads.find(queryReadIndex) == _parent._proccesedReads.end()){
							_parent._partitionReferenceReads.insert(queryReadIndex);
						}
					}
				...
				_parent._partitionRecruitedReads.insert(queryReadIndex);
			}


			//cout << "Nb matches : " << i << endl;
			//_parent.writeAlignments(referenceRead._readIndex, alignedQueryReadsNew);
	...
	class IndexGenomicMinimizersFunctor {
		...
		void operator () (const KminmerList& kminmerList) {
			...
			for(u_int32_t i=0; i<kminmerList._kminmersInfo.size(); i++){
				...
				_parent._kminmer_to_readIndex2.lazy_emplace_l(vec,
				[&readIndex](KminmerPosMap2::value_type& v) { // key exist
				...
				[&vec, &readIndex](const KminmerPosMap2::constructor& ctor) { // key inserted
	...
	KminmerPosMap2 _kminmer_to_readIndex2;
		...
		void operator () (const KminmerList& kminmerList) {
			...
			for(u_int32_t i=0; i<kminmerList._kminmersInfo.size(); i++){
				...
				*/
				...
				_parent._kminmer_to_readIndex2.modify_if(vec, 
					[&readIndex](KminmerPosMap2::value_type& v) { 
	...
	void alignReadsLowDensity(){
		...
		ReadMapper readMapper(_inputDir + "/read_data_init.txt.lowDensity", _alignmentFilename, _minimizerSize, _minimizerDensity_assembly, _minimizerDensity_correction, _usedCoverageForCorrection, _nbCores);
		readMapper.execute();
	...
	class AlignReadsLowDensityFunctor {
		...
		void operator () (const KminmerList& kminmerList) {
			...
			vector<ReadType> alignedQueryReadsNew;
			...
			for(const auto& it : referenceReadIndex_to_anchors2){
				...
				if(readMatchBound._nbMatches < 5) continue;

				alignedQueryReadsNew.push_back(queryReadIndex);
			}
