...
Drep:
mdbg: 23 10 17      1  (k=41, need to determine best k)
hifiasm: 19 14 9      2
metafly: 20 13 23      2

- change hmmer model with 41 COGs
- unique k-mers only for abundance
*/
...
public:
	...
	void loadGraph(){
		...
		_graph->saveUnitigGraph(_inputDir + "/minimizer_graph_u.gfa", _mdbg, _minimizerSize, _nbCores, false);

		//_graph->removeErrorsSimple(_unitigDatas);
		//_graph->clear(0);
		//_graph->compact(false, _unitigDatas);
		//exit(1);
		//Generate unitigs
	...
	void indexReads_read(const vector<u_int64_t>& minimizers, const vector<KmerVec>& kminmers, const vector<ReadKminmer>& kminmersInfos, u_int64_t readIndex){//}, const vector<KmerVec>& kminmers_k3, const vector<ReadKminmer>& kminmersInfos_k3){
		...
		for(const KmerVec& vec : kminmers){
			...
			KmerVec vec2;
			vec2._kmers = {18509800219784382, 26764725113951248, 96424546269098989, 27834495543719520};
			vec2 = vec2.normalize();

			if(vec2 == vec){
				cout << "lala: " << readIndex << endl;
			}
			//if(nodeName == 18347){
			//	cout << "lala: " << readIndex << endl;
			//}
	...
	class ReadCorrectionFunctor {
		...
		void operator () (const KminmerList& kminmerList) {
			...
			*/
			...
			//if(readIndex == 64) print_read = true;		
