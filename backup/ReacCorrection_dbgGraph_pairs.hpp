...
public:
	...
	class Node{
		...
		Node(const KmerVec& minimizer, bool isReversed){
		...
		string toString(){
			return to_string(_minimizer._kmers[0]) +"-" + to_string(_minimizer._kmers[1]);
		}
	...
	class Graph{
		...
		bool _needTopoSort;
		unordered_map<KmerVec, Node*> _nodes;
		...
		void addSequence(const vector<u_int64_t>& minimizers){
			...
			MDBG::getKminmers_complete(2, minimizers, minimizersPos, kminmersInfos, 0, minimizerQualities);
			

			unordered_map<u_int32_t, u_int32_t> readIndex_to_matchCount;


			if(kminmersInfos.size() < 2) return;


			for(size_t i=0; i<kminmersInfos.size()-1; i++){

				KmerVec m1 = kminmersInfos[i]._vec;
				KmerVec m2 = kminmersInfos[i+1]._vec;

				Node* tail = addNode(m1, kminmersInfos[i]._isReversed);
				Node* head = addNode(m2, kminmersInfos[i+1]._isReversed);
			...
			for(size_t i=0; i<kminmersInfos.size(); i++){
				Node* node = addNode(kminmersInfos[i]._vec, kminmersInfos[i]._isReversed);
				node->_abundance += 1;
			}
		}

		Node* addNode(const KmerVec& minimizer, bool isReversed){
			...
			Node* node = new Node(minimizer, isReversed);
		...
		void save(const string& outputFilename, const vector<KmerVec>& readMinimizers){
		...
		vector<u_int64_t> computeConsensus(const vector<KmerVec>& readMinimizers){

			unordered_set<KmerVec> readKminmers;
			for(const KmerVec& vec : readMinimizers){
			...
			for(const KmerVec& vec : readMinimizers){
				...
				PathWeight path = computePathWeight(_nodes[vec], readKminmers);
				
				if(path._weight > bestPath._weight){
					bestPath = path;
				}
			}
			...
			for(size_t i =0; i<bestPath._path.size(); i++){

				Node* node = bestPath._path[i];
				//cout << "\t" << node->toString() << " " << node->_isReversed << endl; 

				if(i == 0){

					vector<u_int64_t> minimizers = node->_minimizer._kmers;

					if(node->_isReversed){
						std::reverse(minimizers.begin(), minimizers.end());
					}

					minimizerPath.push_back(minimizers[0]);
					minimizerPath.push_back(minimizers[1]);
				}
				else{
		...
		PathWeight computePathWeight(Node* startNode, unordered_set<KmerVec>& readKminmers){
			...
			while (true) {
				...
				if(isVisited.find(currentNode) != isVisited.end()) break;
				...
				for(Edge* successor : currentNode->_successors){
					...
					//if(successor->_weight <= 2) continue;
	...
	*/
	...
	class ReadCorrectionFunctor {
		...
		std::unique_ptr<spoa64::AlignmentEngine> _alignmentEngineLocal;// = spoa::AlignmentEngine::Create(spoa::AlignmentType::kNW, 3, -5, -4);
		...
		ReadCorrectionFunctor(const ReadCorrectionFunctor& copy) : _parent(copy._parent){
			...
			_alignmentEngineLocal = spoa64::AlignmentEngine::Create(spoa64::AlignmentType::kSW, 3, -5, -4);
		}
		...
		void operator () (const KminmerList& kminmerList) {
			_parent.correctRead(kminmerList, _alignmentEngine, _alignmentEngineLocal);
		}
	};
	...
	void correctRead(const KminmerList& kminmerList, const std::unique_ptr<spoa64::AlignmentEngine>& al, const std::unique_ptr<spoa64::AlignmentEngine>& alLocal){
		...
		vector<u_int64_t> correctedReadMinimizers = correctWindow(0, readIndex, kminmerList._readMinimizers, kminmerList._readQualities, _print_debug, al, alLocal);
	...
	vector<u_int64_t> correctWindow(size_t windowIndex, u_int64_t readIndex, const vector<u_int64_t>& readMinimizers, const vector<u_int8_t>& readQualities, bool print_debug, const std::unique_ptr<spoa64::AlignmentEngine>& al, const std::unique_ptr<spoa64::AlignmentEngine>& alLocal){
		...
		*/
		...
		vector<u_int64_t> consensus = dbgGraph->computeConsensus(readKminmers);
		/*
		if(_print_debug){
		...
		*/
		...
		//vector<u_int64_t> consensusTrimmed = trimCorrectedPath(readMinimizers, consensus, alLocal);


		if(_print_debug){

			dbgGraph->save(_inputDir + "/poaGraph.gfa", readKminmers);
			...
			for(u_int64_t m : consensus){
				cout << "\t\t" << m << endl;
			}
			
			//cout << endl << "\tCorrected sequence trimmed: " << endl;
			//for(u_int64_t m : consensusTrimmed){
			//	cout << "\t\t" << m << endl;
			//}

			cout << readMinimizers.size() << " " << consensus.size() << endl;
			//cout << "\tdone" << endl;
