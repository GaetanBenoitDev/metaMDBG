...
#ifndef SPOA_GRAPH_HPP_64
#define SPOA_GRAPH_HPP_64
...
using Alignment = std::vector<std::pair<std::int64_t, std::int64_t>>;
...
class Graph {
 public:
  ...
  struct Node {
   public:
    Node(std::uint64_t id, std::uint64_t code);
    ...
    Node* Successor(std::uint64_t label) const;
    ...
    std::vector<uint64_t> _readIndexes;
  ...
  struct Edge {
   public:
    Edge(Node* tail, Node* head, std::uint64_t label, std::uint64_t weight);
    ...
    void AddSequence(std::uint64_t label, std::uint64_t weight = 1);
  ...
  std::uint64_t coder(std::uint64_t c) const {

    int val = 0;
    auto it = coder_.find(c);
    if (it != coder_.end()){
      return it->second;
    }
  ...
  std::uint64_t decoder(std::uint64_t code) const {
  ...
  //std::vector<std::string> GenerateMultipleSequenceAlignment(bool include_consensus = false);
  std::vector<std::vector<uint64_t>> GenerateMultipleSequenceAlignment(bool include_consensus = false);

  std::string GenerateConsensus();
  std::vector<std::uint64_t> GenerateConsensusVec(std::vector<std::uint64_t>& summary,
      bool verbose = false);
  ...
  void save(Archive& archive) const {  // NOLINT
    ...
    std::vector<std::pair<std::uint64_t, std::uint64_t>> aligned_nodes;
    ...
    std::vector<std::uint64_t> rank_to_node_id;
    for (const auto& it : rank_to_node_) {
      rank_to_node_id.emplace_back(it->id);
    }

    std::vector<std::uint64_t> consensus;
  ...
  void load(Archive& archive) {  // NOLINT
    ...
    for (std::uint64_t i = 0; i < connections.size(); ++i) {
  ...
  Node* AddNode(std::uint64_t code);

  void AddEdge(Node* tail, Node* head, std::uint64_t weight);
  ...
  std::vector<std::uint64_t> InitializeMultipleSequenceAlignment(
      std::uint64_t* row_size = nullptr) const;

public:
  std::uint64_t num_codes_;
  std::unordered_map<std::int64_t, std::int64_t> coder_;
  std::unordered_map<std::int64_t, std::int64_t> decoder_;
