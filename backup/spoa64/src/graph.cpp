...
Graph::Node::Node(std::uint64_t id, std::uint64_t code)
    : id(id),
      code(code),
      inedges(),
      outedges(),
...
Graph::Node* Graph::Node::Successor(std::uint64_t label) const {
...
std::uint64_t Graph::Node::Coverage() const {
...
void Graph::Edge::AddSequence(std::uint64_t label, std::uint64_t w) {
...
Graph::Node* Graph::AddNode(std::uint64_t code) {
  nodes_.emplace_back(new Node(nodes_.size(), code));
  return nodes_.back().get();
}

void Graph::AddEdge(Node* tail, Node* head, std::uint64_t weight) {
...
Graph::Node* Graph::AddSequence(
  ...
  for (std::uint64_t i = begin; i < end; ++i) {
    auto curr = AddNode(coder(sequence[i]));
...
Graph::Node* Graph::AddSequence(
  ...
  Node* prev = nullptr;
  for (std::uint64_t i = begin; i < end; ++i) {
    //std::cout << coder_.size() << std::endl;
    //std::cout << sequence[i] << " " << coder_[sequence[i]] << std::endl;
    auto curr = AddNode(coder(sequence[i]));
...
void Graph::performCorrection(
    const Alignment& alignment,
    const std::vector<std::uint64_t>& sequence, std::uint64_t sequence_len, u_int64_t& startNode, u_int64_t& endNode) {

      //std::cout << std::endl << "Perform graph correction" << std::endl;
      //std::cout << "Add align: " << sequence_len << std::endl;
  std::vector<std::uint64_t> weights(sequence_len, 1);
  ...
  for (std::uint64_t i = 0; i < sequence_len; ++i) {
    if (coder(sequence[i]) == -1) {
  ...
  for (const auto& it : alignment) {
    if (it.second != -1) {
      if (it.second < 0 || it.second >= static_cast<std::int64_t>(sequence_len)) {  // NOLINT
        throw std::invalid_argument(
  ...
  for (const auto& it : alignment) {
    ...
    std::uint64_t code = coder(sequence[it.second]);
    Node* curr = nullptr;
    if (it.first == -1) {
    ...
    if(prev != nullptr){
      ...
      //std::cout << prev->id << "-" << decoder(prev->code) << " " << prev->Coverage() << std::endl;
      //for(Edge* edge : prev->outedges){
      //  std::cout << "\t" << edge->head->id << "-" << decoder(edge->head->code) << " " << edge->weight << std::endl;
      //}
        //std::cout << decoder(prev->code) << " -> " << decoder(curr->code)  << std::endl;
  ...
  if (last) {
      ...
      //std::cout << "a ne pas oublier: " << last->id << std::endl;
    //endNode = last->id;
...
void Graph::AddAlignment(
    ...
    std::vector<std::uint64_t>& weights) {
      ...
      //for(size_t i=0; i<weights.size(); i++) weights[i] = 1;
      //std::cout << "Add align: " << sequence_len << std::endl;
  //std::vector<std::uint64_t> weights(sequence_len, weight);
...
void Graph::AddAlignment(
    const Alignment& alignment,
    const char* sequence, std::uint64_t sequence_len,
    const char* quality, std::uint64_t quality_len) {
  std::vector<std::uint64_t> weights;
  for (std::uint64_t i = 0; i < quality_len; ++i) {
...
void Graph::TopologicalSort() {
  ...
  std::vector<std::uint64_t> marks(nodes_.size(), 0);
...
std::vector<std::uint64_t> Graph::InitializeMultipleSequenceAlignment(
    std::uint64_t* row_size) const {
  std::vector<std::uint64_t> dst(nodes_.size());
  std::uint64_t j = 0;
  for (std::uint64_t i = 0; i < rank_to_node_.size(); ++i, ++j) {
...
std::vector<std::string> Graph::GenerateMultipleSequenceAlignment(
  ...
  for (std::uint64_t i = 0; i < sequences_.size(); ++i) {
    std::string row(row_size, '-');
    auto it = sequences_[i];
    while (true) {
      row[node_id_to_column[it->id]] = decoder(it->code);
...
*/
std::vector<std::vector<uint64_t>> Graph::GenerateMultipleSequenceAlignment(
    bool include_consensus) {
  std::uint64_t row_size = 0;
  auto node_id_to_column = InitializeMultipleSequenceAlignment(&row_size);
  ...
  for (std::uint64_t i = 0; i < sequences_.size(); ++i) {
    std::vector<uint64_t> row(row_size, -1);
    auto it = sequences_[i];
    while (true) {
      row[node_id_to_column[it->id]] = decoder(it->code);
      if (!(it = it->Successor(i))) {
...
std::vector<std::uint64_t> Graph::GenerateConsensusVec(std::vector<std::uint64_t>& summary,
    bool verbose) {
  TraverseHeaviestBundle();
  //std::string dst{};
  ...
  for(size_t i=0; i<consensus_.size()-1; i++){

    Graph::Node* node = consensus_[i];
    Graph::Node* nextNode = consensus_[i+1];

    for(const auto& edge : node->outedges){
      if(edge->head == nextNode){
        summary.push_back(edge->weight);
        //std::cout << "mdr: " << summary.size() << std::endl;
        break;
      }
    }
  ...
  */
  ...
  for (const auto& it : consensus_) {
    result.push_back(decoder(it->code));
  }
...
std::string Graph::GenerateConsensus(
  ...
  if (!verbose) {
    ...
    for (std::uint64_t i = 0; i < sequences_.size(); ++i) {
      ...
      std::uint64_t c = 0, p, column = node_id_to_column[it->id];
      bool is_gap = false;
      while (true) {
...
void Graph::TraverseHeaviestBundle() {
  ...
  if (!max->outedges.empty()) {
    std::vector<std::uint64_t> node_id_to_rank(nodes_.size(), 0);
    for (std::uint64_t i = 0; i < rank_to_node_.size(); ++i) {
...
Graph::Node* Graph::BranchCompletion(
  ...
  Node* max = nullptr;
  for (std::uint64_t i = rank + 1; i < rank_to_node_.size(); ++i) {
    auto it = rank_to_node_[i];
    (*scores)[it->id] = -1;
...
void Graph::PrintDot(const std::string& path) const {
  ...
  std::vector<std::int64_t> consensus_rank(nodes_.size(), -1);
  std::int64_t rank = 0;
  for (const auto& it : consensus_) {
  ...
  for (const auto& it : nodes_) {
    os << "  " << it->id << "[label = \"" << it->id << " - "
       << static_cast<char>(decoder(it->code)) << "\"";
