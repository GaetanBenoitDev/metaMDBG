...
public:
    ...
    void execute (){
		...
		//derepContigs(_readPartitionDir + "/contigs_polished.fasta.gz", _readPartitionDir + "/contigs_derep.fasta.gz", 1);
		//exit(1);
		...
		*/
		...
		//trimContigs(outputContigFilename_polished, outputContigFilename_trimmed, minimapBatchSize);
	...
	class AlignReadFunctor {
		...
		void operator () (const KminmerList& kminmerList) {
			...
			if(kminmerList._readIndex % 100000 == 0){
				cout << "Align reads: " << kminmerList._readIndex << endl;
			}
	...
	void loadReads(const string& readFilename){
		...
		cout << "\tload reads todo:  load only selected reads" << endl; 
		cout << "\tload reads todo:  pas besoin de toutes l'information + on est en low density (genre snpmers etc)" << endl;
	...
	void createBaseContigs(const string& contigFilename){
		...
		for(u_int32_t i=0; i<_nbPartitions; i++){
			...
			cout << "Process partition: " << i << "/" << _nbPartitions << endl;
		...
		void readPathsToContigs(const KminmerList& kminmerList, const vector<ReadPath>& readPaths, auto& readsVsReadsAlignments, auto& readIndex_to_alignment){
			...
			for(size_t pathIndex=0; pathIndex < readPaths.size(); pathIndex++){
				...
				#pragma omp critical(writeContig)
				{
					...
					cout << "\tWrite contig: " << contigSequence.size() << endl;
	...
	void derepContigs(const string& inputContigFilename, const string& outputContigFilename, const int minimapBatchSize){
		...
		ContigDerep contigDerep(inputContigFilename, outputContigFilename, true, minimapBatchSize, _inputDir, _readPartitionDir, 0.9, _minContigLength, _nbCores);
		contigDerep.execute();
