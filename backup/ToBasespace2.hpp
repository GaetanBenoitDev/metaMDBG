...
public:
    ...
    void execute (){
		...
		cout << "Tobasepace: can be improved: trim contigs based of pair-matches before alignment" << endl;
	...
	*/
	...
	void alignRead(const KminmerList& kminmerList, MinimizerAligner* minimizerAligner){
		...
		if(kminmerList._readIndex % 10000 == 0) cout << "Aligning read: " << kminmerList._readIndex << endl;
		...
		MDBG::getKminmers_complete(_kminmerSize, queryRead._minimizers, queryRead._minimizersPos, kminmersInfos, queryRead._readIndex, queryRead._qualities);
		...
		for(const auto& it : referenceReadIndex_to_anchors2){
			...
			//cout << contigIndex << " " << queryRead._readIndex << endl;
			const vector<MinimizerType>& contigMinimizers = _mContigs[contigIndex];

			/*
			int64_t actualStart = -1;
			int64_t actualEnd = -1;
			...
			*/
			...
			int64_t startQueryPositionIndex = getStartQueryPositionIndex(contigMinimizers, readMatchBound._minIndex, 5000, queryReadMinimizerPositionMap);
			int64_t endQueryPositionIndex = getEndQueryPositionIndex(contigMinimizers, readMatchBound._maxIndex, 5000, queryReadMinimizerPositionMap);

			int64_t overHangOffset = 5000*_minimizerDensity;
			...
			cout << "----" << endl;
			for(size_t i=0; i<subContigMinimizers_forward.size(); i++){
				cout << i << "\t" << subContigMinimizers_forward[i] << endl;
			}
			*/
			MinimizerAligner::Alignment alignment_forward = minimizerAligner->performAlignment(queryRead._minimizers, subContigMinimizers_forward);
			MinimizerAligner::Alignment alignment_reverse = minimizerAligner->performAlignment(queryRead._minimizers, subContigMinimizers_reverse);

			int64_t score_forward = getAlignmentScore(alignment_forward, queryRead._minimizers, subContigMinimizers_forward);
			int64_t score_reverse = getAlignmentScore(alignment_reverse, queryRead._minimizers, subContigMinimizers_reverse);

			//cout << score_forward << " " << score_reverse << endl;
			//getchar();

			if(score_forward < 2 && score_reverse < 2) continue;

			if(score_forward > score_reverse){
				extractAlignment(alignment_forward, queryRead._readIndex, queryRead._minimizers, contigIndex, subContigMinimizers_forward, score_forward, false, startQueryPositionIndex);
			}
			else{
				extractAlignment(alignment_reverse, queryRead._readIndex, queryRead._minimizers, contigIndex, subContigMinimizers_reverse, score_reverse, true, startQueryPositionIndex);
			}
	...
	int64_t getAlignmentScore(const MinimizerAligner::Alignment& alignment, const vector<MinimizerType>& minimizers1, const vector<MinimizerType>& minimizers2){
		...
		for(const auto& it : alignment){
			...
			if(it.first == -1){
				...
				if(minimizers1[it.first] == minimizers2[it.second]){
					nbMatches += 1;
				}
				else{
					nbMissmatches += 1;
		...
		return nbMatches - nbMissmatches - nbDeletions - nbInsertions;
	}
	...
	*/
	...
	void extractAlignment(const spoa64::Alignment& alignment, u_int32_t readIndex, const vector<MinimizerType>& readMinimizers, u_int32_t contigIndex, const vector<MinimizerType>& contigMinimizers, int64_t alignmentScore, bool isReversed, int64_t subContigStartIndex){
		...
		for (size_t i=0; i<alignment.size()-1; i++) {
			...
			if(readPosition == -1){ //insert in
				...
				if(prevReadPosition != -1 && prevContigPosition != -1){
					...
					prevContigPositionNorm += subContigStartIndex;
					indexMatchingPosition(readMinimizers[prevReadPosition], readMinimizers[readPosition], {readIndex, (u_int32_t)prevReadPosition, alignmentScore}, {contigIndex, prevContigPositionNorm});
				}
	...
	*/
	...
	void createBaseContigs(const string& contigFilename){
		...
		KminmerParser parser(contigFilename, _minimizerSize, _kminmerSize, false, false);
		auto fp = std::bind(&ToBasespace2::createBaseContigs_read, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4);
