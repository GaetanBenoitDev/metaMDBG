...
public:
	...
	ofstream _mappedReadFile;
	ofstream _outputReadFile;
	unordered_set<u_int32_t> _writtenReads;
    ...
    void execute (){
		...
		*/
		...
		_mappedReadFile = ofstream(_inputDir + "/contig_to_read.bin");
		_outputReadFile = ofstream(_inputDir + "/contig_to_read.fasta");
		...
		_mappedReadFile.close();
		_outputReadFile.close();
		//delete _mdbg;
		/*
	...
	*/
	...
	unordered_map<u_int32_t, string> _readIndex_to_seq;
	...
	class ExtractKminmerSequenceFunctor{
		...
		void operator () (const Read& read) {
			...
			#pragma omp critical
			{
				_toBasespace._readIndex_to_seq[readIndex] = read._seq;
			}
	...
	class CreateBaseContigsFunctor {
		...
		void operator () (const KminmerList& kminmerList) {
			...
			for(u_int32_t i=0; i<kminmerList._kminmersInfo.size(); i++){
				...
				if(_toBasespace._writtenReads.find(readPositionMatch._readIndex) == _toBasespace._writtenReads.end()){
					cout << readIndex << " " << readPositionMatch._readIndex << endl;
					_toBasespace._writtenReads.insert(readPositionMatch._readIndex);
					_toBasespace._mappedReadFile.write((const char*)&readIndex, sizeof(readIndex));
					_toBasespace._mappedReadFile.write((const char*)&readPositionMatch._readIndex, sizeof(readPositionMatch._readIndex));

					_toBasespace._outputReadFile << ">read_" << readPositionMatch._readIndex << endl;
					_toBasespace._outputReadFile << _toBasespace._readIndex_to_seq[readPositionMatch._readIndex] << endl;
				}
